<div class="history-data-display">
    <div class="history-data-separator-two-column">
        <div class="three-column-table-container">
            <table>
                <tbody>
                    <tr>
                    ...
                    data.addRows([
                        <?php
                            ...
                            for($i = 0; $i < count($data["timestamp"]); $i++)
                            {
                                ...
                                // *** SECURITY FIX: JSON encode the string literal for safe JS embedding ***
                                if($data["pass"][$i] === "Pass")
                                {
                                    $style = json_encode('point { size: 10; shape-type: star; fill-color: magenta; }');
                                }
                                else
                                {
                                    $style = "null";
                                ...
                                // *** SECURITY FIX: Use floatval() for score ***
                                $dataRows[] = "[new Date(" . $datestr . "), " . floatval($score) . ", " . $style . "]";
